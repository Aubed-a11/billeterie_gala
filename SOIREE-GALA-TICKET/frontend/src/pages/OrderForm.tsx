...
export const OrderForm: React.FC<OrderFormProps> = ({ onBack, onSuccess }) => {
  ...
  if (step === 2 && createdOrderId) {
    const amountDHS = (packInfo?.price || 200) * (watch('nombrePersonnes') || 1);
            ...
            <div className="glass-card p-6 rounded-3xl border-gala-gold/30">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-start gap-4 text-sm">
                  <span className="text-slate-400 shrink-0">Ticket sélectionné</span>
                ...
                <div className="flex justify-between items-start gap-4 text-sm">
                  ...
                  <span className="text-white font-medium text-right">{packInfo.price.toLocaleString()} DH</span>
                </div>
                <div className="pt-4 border-t border-slate-700/50 flex justify-between items-center">
                  <span className="text-white font-bold">Total</span>
                  <span className="text-2xl font-serif text-gala-gold">{(packInfo.price * nombrePersonnes).toLocaleString()} DH</span>
                </div>
              </div>
            </div>
          </div>
